...
class Pi0(_model.BaseModel):
    ...
    def sample_actions_rtc(
        ...
        *,
        ...
        def v_t_fn(x_t: jax.Array, t: float) -> jax.Array:
            # Add batch dimension since embed_suffix expects batched inputs
            x_t_batched = x_t[None, ...] if x_t.ndim == 2 else x_t
            t_batched = jnp.array([t]) if jnp.isscalar(t) else jnp.broadcast_to(t, batch_size)

            suffix_tokens, suffix_mask, suffix_ar_mask = self.embed_suffix(observation, x_t_batched, t_batched)
            # `suffix_attn_mask` is shape (b, suffix_len, suffix_len) indicating how the suffix tokens can attend to each
            # other
            suffix_attn_mask = make_attn_mask(suffix_mask, suffix_ar_mask)
            # `prefix_attn_mask` is shape (b, suffix_len, prefix_len) indicating how the suffix tokens can attend to the
            ...
            return v_t[0] if x_t.ndim == 2 else v_t
        ...
        def rtc_step(carry):
            ...
            def pinv_corrected_velocity(x_t, y, t):
                ...
                prefix_weights = get_prefix_weights(
                    inference_delay, prefix_attention_horizon, prefix_actions.shape[1], "exp"
                )
                error = (y - x_0) * prefix_weights[:, None]
                ...
                guidance_weight = jnp.minimum(-jnp.log(t), max_guidance_weight)

                return v_t - guidance_weight * pinv_correction

            v_t = pinv_corrected_velocity(x_t, prefix_actions, time)
            return x_t + dt * v_t, time + dt
        ...
        # x_0, _ = jax.lax.while_loop(cond, step, (noise, 1.0))
        # prefix_weights = get_prefix_weights(inference_delay, prefix_attention_horizon, self.action_horizon, "linear")
        # jax.debug.print("[DEBUG] prefix_actions: {}", prefix_actions)
        # jax.debug.print("[DEBUG] prefix_weights: {}", prefix_weights)
        # jax.debug.print("[DEBUG] x_0: {}", x_0)
        # ones = jnp.ones(prefix_weights.shape)
        # x_0 = prefix_actions * prefix_weights[:, None] + x_0 * (ones - prefix_weights)[:, None]
        # jax.debug.print("[DEBUG] out: {}", x_0)
        return x_0
    ...
    #                 inference_delay, prefix_attention_horizon, prefix_actions.shape[1], "exp"
    #             )
    #             # jax.debug.print("[DEBUG] y shape: {}, first 5: {}", y.shape, y[:5, 0])
    #             # jax.debug.print("[DEBUG] prefix_weights: {}", prefix_weights)
    #             error = (y - x_0) * prefix_weights[:, None]
    #             pinv_correction = vjp_fun(error)[0]
    #             # constants from paper
    ...
    #             v_t_step, x_t, time, prefix_actions, inference_delay, prefix_attention_horizon, max_guidance_weight
    #         )
    ...
    #     x_0, final_time = jax.lax.while_loop(cond, rtc_step, (noise, 1.0))
