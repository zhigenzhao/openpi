...
@dataclasses.dataclass(frozen=True)
class LeRobotDualArxR5DataConfig(DataConfigFactory):
    """
    This config is used to configure transforms that are applied at various parts of the data pipeline.
    For your own dataset, you can copy this class and modify the transforms to match your dataset based on the
    comments below.
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[
                arx_dual_arm_policy.ArxR5Inputs(action_dim=model_config.action_dim, model_type=model_config.model_type)
            ],
            outputs=[arx_dual_arm_policy.ArxR5Outputs()],
        )
...
@dataclasses.dataclass(frozen=True)
class LeRobotDualArxR5LeRobotDataConfig(DataConfigFactory):
    """
    This config is used to configure transforms that are applied at various parts of the data pipeline.
    For your own dataset, you can copy this class and modify the transforms to match your dataset based on the
    comments below.
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        data=LeRobotArxR5DataConfig(
            repo_id="kelvinzhaozg/arx_single_arm_block_pick_and_place_gripper_normalized",
            base_config=DataConfig(prompt_from_task=True),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi0_base/params"),
    ...
    TrainConfig(
        name="pi0_arx_dual_low_mem_finetune",
        # Here is an example of loading a pi0 model for LoRA fine-tuning.
        model=pi0.Pi0Config(paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
    ...
    TrainConfig(
        name="pi0_arx_dual_low_mem_finetune_multi_step",
        # Here is an example of loading a pi0 model for LoRA fine-tuning.
        model=pi0.Pi0Config(paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
        data=LeRobotDualArxR5DataConfig(
            repo_id="kelvinzhaozg/arx_dual_arm_carpet_fold_combined",
            base_config=DataConfig(prompt_from_task=True),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi0_base/params"),
    ...
    TrainConfig(
        name="pi0_arx_dual_low_mem_finetune_carpet_separation",
        # Here is an example of loading a pi0 model for LoRA fine-tuning.
        model=pi0.Pi0Config(paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
        data=LeRobotDualArxR5LeRobotDataConfig(
            repo_id="kelvinzhaozg/arx_dual_carpet_separation_lerobot",
            base_config=DataConfig(prompt_from_task=True),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi0_base/params"),
        num_train_steps=80000,
